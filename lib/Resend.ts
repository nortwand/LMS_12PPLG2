...
const FROM_EMAIL = "Studify <onboarding@resend.dev>";

export async function sendOtpEmail(toEmail: string, nama: string, otp: string) {
  const { data, error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: toEmail,
    subject: "Kode OTP Reset Password Studify",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2 style="color: #658864;">Studify</h2>
        <p>Halo <strong>${nama}</strong>,</p>
        <p>Berikut kode OTP untuk mengubah password akun Studify anda:</p>
        <div style="background: #FAF6EE; border-radius: 12px; padding: 24px; text-align: center; margin: 16px 0;">
